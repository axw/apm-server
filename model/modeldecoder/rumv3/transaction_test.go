...
func TestDecodeMapToTransactionModel(t *testing.T) {
	...
	t.Run("overwrite-user", func(t *testing.T) {
		...
		tr := initializedMetadata()
		inputTr.Context.User.Email.Set("test@user.com")
		mapToTransactionModel(&inputTr, time.Now(), &tr)
		assert.Equal(t, "test@user.com", tr.User.Email)
		assert.Zero(t, tr.User.ID)
		assert.Zero(t, tr.User.Name)
	})
	...
	t.Run("page.URL", func(t *testing.T) {
		...
		var tr model.APMEvent
		mapToTransactionModel(&inputTr, time.Now(), &tr)
		assert.Equal(t, "https://my.site.test:9201", tr.Transaction.Page.URL.Full)
		assert.Equal(t, "https://my.site.test:9201", tr.Transaction.URL.Full)
		assert.Equal(t, 9201, tr.Transaction.Page.URL.Port)
		assert.Equal(t, "https", tr.Transaction.Page.URL.Scheme)
	})
	...
	t.Run("page.referer", func(t *testing.T) {
		...
		var tr model.APMEvent
		mapToTransactionModel(&inputTr, time.Now(), &tr)
		assert.Equal(t, "https://my.site.test:9201", tr.Transaction.Page.Referer)
		assert.Equal(t, "https://my.site.test:9201", tr.Transaction.HTTP.Request.Referrer)
	})
