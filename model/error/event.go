...
func decodeEvent(input interface{}, err error) (*Event, map[string]interface{}, error) {
	...
	for ex != nil {
		...
		*exOut = exception
		...
		ex = decoder.MapStr(ex, "cause")
		if ex != nil {
			exOut = &exception.Cause
		}
	}
...
func (e *Event) addException(tctx *transform.Context) {
	...
	for exception != nil {
		ex := common.MapStr{}
		utility.Add(ex, "message", exception.Message)
		utility.Add(ex, "module", exception.Module)
		utility.Add(ex, "attributes", exception.Attributes)
		utility.Add(ex, "type", exception.Type)
		utility.Add(ex, "handled", exception.Handled)
...
func (e *Event) calcGroupingKey() string {
	...
	var st m.Stacktrace
	for ex := e.Exception; ex != nil; ex = ex.Cause {
		k.add(ex.Type)
		if len(st) == 0 {
			st = ex.Stacktrace
		}
