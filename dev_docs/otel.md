...
server is created in the [`apmserver`](../internal/apmserver) package, with requests flowing through various
gRPC interceptors defined under [`apmserver/interceptors`](../internal/apmserver/interceptors) for authentication,
logging, rate limiting, etc.

We register OTLP/gRPC services for receiving traces, metrics, and logs with the gRPC server in
[`apmserver/otlp`](../internal/apmserver/otlp). The business logic for handling these events lives in the type
