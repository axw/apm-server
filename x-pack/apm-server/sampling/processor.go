...
func (p *Processor) processTransaction(event *model.APMEvent) (report, stored bool, _ error) {
	...
	if event.Transaction.ParentID != "" {
		...
		return false, true, p.storage.WriteEvent(event)
	}
...
func (p *Processor) processSpan(spanEvent *model.APMEvent) (report, stored bool, _ error) {
	traceSampled, err := p.storage.IsTraceSampled(spanEvent.Span.TraceID)
	if err != nil {
		if err == eventstorage.ErrNotFound {
			// Tail-sampling decision has not yet been made, write span to local storage.
			return false, true, p.storage.WriteEvent(spanEvent)
		}
		return false, false, err
	}
			...
			if n := len(events); n > 0 {
				...
				if remoteDecision {
					...
					for _, event := range events {
						if err := p.storage.DeleteEvent(&event); err != nil {
							return errors.Wrap(err, "failed to delete event from local storage")
						}
					}
				}
