...
func main() {
	...
	consumer, err := kafkaconsumer.New(kafkaconsumer.Config{
		...
		Topics:          []string{"apm-destmetrics-events"},
		ConsumerGroupID: "apm-destmetrics",
	})
	if err != nil {
		log.Fatal(err)
	}
	...
	agg, err := spanmetrics.NewAggregator(spanmetrics.AggregatorConfig{
		BatchProcessor: modelprocessor.Chained{logEventsProcessor, producer},
		MaxGroups:      10000,
		Interval:       10 * time.Second,
		Logger:         logp.NewLogger("destmetrics"),
	})
	if err != nil {
		log.Fatal(err)
	}
	...
	// to the input batch.)
	if err := consumer.Consume(ctx, agg); err != nil {
		log.Fatal(err)
	}
}
